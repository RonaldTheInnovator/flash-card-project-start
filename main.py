...
def next_card():
    ...
    window.after_cancel(flip_timer)  #TO CANCEL 3 SECONDS

    current_card = random.choice(to_learn)
    canvas.itemconfig(card_title,text="French",fill="black")
    canvas.itemconfig(card_word, text= current_card["French"],fill="black")
    canvas.itemconfig(card_background, image=front_card_image)
    flip_timer = window.after(3000, func=flip_card) #SET NEW FLIP TIMER
...
window.config(padx=50,pady=50,width=200,bg=BACKGROUND_COLOR)

flip_timer = window.after(3000,func=flip_card) #THIS IS 3 SECONDS OR 3000 MILLI SECONDS
